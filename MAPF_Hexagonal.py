...
class TimeExpandedNetworks:
    ...
    def build(self, num_initial_nodes):
        self.assembled.append(0)
    ...
    def customized_dijkstra(self):
        ...
        while priority_queue:
            ...
            for neighbor, weight in graph[current_node].items():
                ...
                distance = current_distance + modified_weight
    ...
    def extend_ten(self, neighbors, t_final, t_nonzero):
        ...
        if len(self.assembled) < self.cg.num_nodes:
            if t_nonzero >= (len(self.assembled) - 1) * self.delta_t:
                self.tenGraph.add_edge('source', '0_' + str(t_nonzero), capacity=1, flow=0, cost=t_nonzero)
            else:
                self.tenGraph.add_edge('source', '0_' + str((len(self.assembled) - 1) * self.delta_t),
                                       capacity=1, flow=0, cost=t_nonzero)
                ...
                for t in range(t_final + 1, self.T):
                    if t < self.T - 1:
                        ...
                        for n in neighbors_1:
                            if n in self.assembled:
                                if self.tenGraph.has_node('w_' + str(t) + "_" + str(i) + "_" + str(n)):
                                    continue
                                if self.tenGraph.has_node('w_' + str(t) + "_" + str(n) + "_" + str(i)):
                                    continue
                                self.tenGraph.add_node('w_' + str(t) + "_" + str(i) + "_" + str(n),
                                                       pos=(t + 0.6, -0.5 * (i + n + 0.2)))
                                self.tenGraph.add_node('w_' + str(t) + "'_" + str(i) + "_" + str(n),
    ...
    def max_flow(self):
        ...
        paths_re = []
        while True:
            path, path_cost = self.shortest_path()
            if path is None:
                break
            else:
                paths.append(path)
                ...
                # update flow equals to delete the edges due to flow limit is 1
...
def mapf_demo(seed=0, T=50):
    random.seed(seed)

    start = time.time()

    # Build Graph G of the final target configuration
    cg = ConnectionGraph(37)
    cg.configFullConnection()

    # Build the initial TEN
    ten = TimeExpandedNetworks(cg, T, 1)
    ten.build(7)

    # Start flow algorithm
